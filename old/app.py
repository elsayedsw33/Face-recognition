...
def load_models():
    ...
    # Anti-spoofing ONNX model (ONNX Runtime)
...
def predict_face(frame, face, models):
    """Run anti-spoofing, then recognition (only if real) for one detected face.
    ...
    is_real = run_anti_spoofing(face_crop, models)
...
# =========================
# Image prediction (used by BOTH upload and camera modes)
# =========================

def predict_image(image_bgr, models):
    ...
    with _inference_lock:
        ...
        if faces is not None:
            for face in faces:
                result = predict_face(image_bgr, face, models)
...
def decode_image(uploaded_file):
    """Convert a Streamlit uploaded/camera file to a BGR OpenCV image."""
...
def show_results(results):
    ...
    for i, result in enumerate(results, start=1):
        ...
        if result["name"] == "Unknown":
            st.warning("Face is real but not recognized (Unknown).")
        else:
...
def run_and_show(image_bgr, models):
    with st.spinner("Analyzing..."):
        results = predict_image(image_bgr, models)
    show_results(results)


def run_live_camera(models):
    """Live camera: the video stays clean, predictions update below it."""
